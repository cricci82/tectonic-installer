...
module "ignition_main_etcd" {
  source = "../../modules/aws/ignition_main"

  instance_count  = "${length(data.template_file.etcd_hostname_list.*.id)}"
  //source_url_list = "s3://${aws_s3_bucket_object.ignition_etcd[count.index].bucket}/${aws_s3_bucket_object.ignition_etcd.key}"
  bucket          = "${aws_s3_bucket_object.ignition_etcd.0.bucket}"
  keys            = "${aws_s3_bucket_object.ignition_etcd.*.key}" 
  content         = "${aws_s3_bucket_object.ignition_etcd.*.content}"
  source_url_list = ""
  verification = ""
}
...
module "etcd" {
  ...
  ignition_main           = "${module.ignition_main_etcd.ignition}"
}
...
module "ignition_main_masters" {
  source = "../../modules/aws/ignition_main"
  bucket = "${aws_s3_bucket_object.ignition_master.bucket}"
  keys = "${list(aws_s3_bucket_object.ignition_master.key)}"
  content = "${list(aws_s3_bucket_object.ignition_master.content)}"
  source_url_list = "s3://${aws_s3_bucket_object.ignition_master.bucket}/${aws_s3_bucket_object.ignition_master.key}"
...
module "ignition_main_workers" {
  ...
  bucket = "${aws_s3_bucket_object.ignition_worker.bucket}"
  keys = "${list(aws_s3_bucket_object.ignition_worker.key)}"
  content = "${list(aws_s3_bucket_object.ignition_worker.content)}"
  source_url_list = "s3://${aws_s3_bucket_object.ignition_worker.bucket}/${aws_s3_bucket_object.ignition_worker.key}"
